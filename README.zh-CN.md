...
### `bin/discordctl.py`
主 CLI，负责 Discord 管理、organize 规划，以及安装到 Claude skill 目录的 agent 流程。
...
- 应从 GitHub 拉取仓库并安装到 Claude skill 目录，而不是依赖无关的本地副本
- 如果有缺失值，优先写入 Claude local settings，使其作为环境变量生效
